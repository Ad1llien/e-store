...
class PaymentSuccessView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if payapl_order_id != "null":
            ...
            if response.status_code == 200:
                paypal_order_data = response.json()#email sending
                subject = f"Order Placed Successfully"
                message = f"Thank you for purchase! \n\n{order}"
                print(order)
                send_mail(
                    subject, 
                    message,
            ...
            if session.payment_status == "paid":
                if order.payment_status == "processing":
                    ...
                    message = (
                        ...
                        f"Thank you for your order with us! Below are the details of your purchase:\n\n"
                        f"Order Summary:\n"
                        ...
                        f"Shipping Address:\n"
                        f"{order.address}, {order.city}, {order.state}, {order.country}\n\n"
                        f"Order Details:\n"
                        f"Subtotal: ${order.sub_total:.2f}\n"
                        f"Shipping Cost: ${order.shipping_amount:.2f}\n"
                        f"Tax: ${order.tax_fee:.2f}\n"
                        f"Service Fee: ${order.service_fee:.2f}\n"
                        f"Discounts Applied: ${order.saved:.2f}\n"
                        f"Total: ${order.total:.2f}\n\n"
                        f"Payment Status: {order.payment_status}\n"
                        f"Order Status: {order.order_status}\n\n"
                        f"If you have any questions or need to update your order, please contact us.\n\n"
                        f"Best regards,\n"
                        f"Your Company Name"
                    )
                    print(order)
